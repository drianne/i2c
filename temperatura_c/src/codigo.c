...
{   
    ...
    float total_temperatura, total_umidade, total_pressao;
    int contador = 0;
    ...
    if( access( "temps.csv", F_OK ) == -1 ) {
        fp=fopen("temps.csv","a+");
        fprintf(fp,"Data/hora, Temperatura, Umidade, Pressão");
        fclose(fp);
    }
    ...
    {        
        ...
        total_temperatura += comp_data.temperature;
        total_pressao += 0.01 * comp_data.pressure;
        total_umidade += comp_data.humidity;
        contador+=1;
        ...
        if(contador == 10){
            ...
            save_into_csv(&fp, timeinfo, total_temperatura/10 , total_umidade/10 , total_pressao/10);
